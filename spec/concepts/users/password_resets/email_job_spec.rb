...
RSpec.describe Users::PasswordResets::EmailJob do
  describe '#perform' do
    it 'generates token and sends it in the email' do
      freeze_time do
        ...
        expect(mail_message.body).to match(%r"users/password_reset/edit\?token=[ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz123456789]{48}")

        expect(user.reload.password_reset_sent_at).to eq(Time.current)
      end
    end
  end
end
