...
RSpec.describe Users::PasswordResets::Update do
  describe '#call' do
    ...
    context 'when user is found by token' do
      context 'when trying to reset password raises an error' do
        it 'raises an error and uncalls rate_limiter' do
          ...
          rate_limiter_key = 'users_password_resets_update:1.1.1.1'
          RateLimiting.redis.set(rate_limiter_key, '0')
          allow_any_instance_of(User).to receive(:reset_password!).and_raise(ActiveRecord::ActiveRecordError)
      ...
      context 'when trying to reset password succeeds' do
        it 'returns success and expires tokens' do
          ...
          expect(result.data[:user].password_reset_at).to be_present
          expect(RateLimiting.redis.get(rate_limiter_key)).to eq('1')
          expect(User.find_by_token(:password_reset, token)).to be_blank
        end
      end
    end
  end
