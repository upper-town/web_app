...
RSpec.describe ServerWebhooks::DeleteOldEventsJob do
  describe '#perform' do
    it 'deletes events in final statuses that have not been updated in the last 3 months' do
      travel_to(Time.zone.parse('2023-06-01T12:50:10'))
      ...
      failed_event_3 = create(:server_webhook_event, status: ServerWebhookEvent::FAILED, updated_at: '2023-03-01T12:50:10')
      failed_event_4 = create(:server_webhook_event, status: ServerWebhookEvent::FAILED, updated_at: '2023-01-01')
      ...
      delivered_event_3 = create(:server_webhook_event, status: ServerWebhookEvent::DELIVERED, updated_at: '2023-03-01T12:50:10')
      delivered_event_4 = create(:server_webhook_event, status: ServerWebhookEvent::DELIVERED, updated_at: '2023-01-01')

      described_class.new.perform

      all_events = ServerWebhookEvent.all
      expect(all_events).not_to include(
        failed_event_3,
        failed_event_4,
        delivered_event_3,
        delivered_event_4,
      )
      expect(all_events).to contain_exactly(
        pending_event_1,
        pending_event_2,
        retry_event_1,
        retry_event_2,
